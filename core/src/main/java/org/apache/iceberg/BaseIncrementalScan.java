...
abstract class BaseIncrementalScan<ThisT, T extends ScanTask, G extends ScanTaskGroup<T>>
  ...
  public ThisT fromSnapshotInclusive(long fromSnapshotId, String branch) {
    ...
    TableScanContext newContext = context().fromSnapshotIdInclusive(fromSnapshotId).useBranch(branch);
  ...
  public ThisT fromSnapshotExclusive(long fromSnapshotId, String branch) {
    ...
    Preconditions.checkArgument(
        table().snapshot(branch) != null, "Cannot find the branch: %s", branch);
    TableScanContext newContext = context().fromSnapshotIdExclusive(fromSnapshotId).useBranch(branch);
    return newRefinedScan(tableOps(), table(), schema(), newContext);
  }
