...
public class TestFlinkSqlNestProjection extends TestFlinkSource {
  ...
  public void testNestProjectionOneStruct() throws Exception {
    ...
    List<Row> result = sql("SELECT id, st.a, st.b FROM t");

    Schema schemaResult = new Schema(
        Types.NestedField.required(1, "id", Types.LongType.get()),
        Types.NestedField.required(2, "st.a", Types.LongType.get()),
    ...
    for (Record sourceRecord : writeRecords) {
      ...
      if (st != null) {
        genericRecord.set(1, ((Record) st).get(0));
    ...
    org.apache.iceberg.flink.TestHelpers.assertRecords(result, expectedRecords, schemaResult);
  }
