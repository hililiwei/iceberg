...
public class TestIcebergStreamWriter {
  ...
  public void testPreCommit() throws Exception {
    IcebergStreamWriter icebergStreamWriter = createIcebergStreamWriter();
  ...
  public void testSnapshotAndRestore() throws Exception {
    ...
    Collection<IcebergStreamWriterState> icebergStreamWriterStates = icebergStreamWriter.snapshotState(1L);
    icebergStreamWriter.prepareCommit();
    Assert.assertEquals(icebergStreamWriterStates.size(), 1);

    IcebergStreamWriter icebergStreamWriterRestore =
        createIcebergStreamWriter().restoreWriter(icebergStreamWriterStates);

    List<IcebergStreamWriterState> icebergStreamWriterStatesRestore =
  ...
  public void testTableWithTargetFileSize() throws Exception {
    ...
    try (IcebergStreamWriter icebergStreamWriter = createIcebergStreamWriter()) {
      for (RowData row : rows) {
        icebergStreamWriter.write(row, new ContextImpl());
      }
      Collection<IcebergFlinkCommittable> icebergFlinkCommittables = icebergStreamWriter.prepareCommit();
      WriteResult result = icebergFlinkCommittables.stream().findFirst().get().committable();
      Assert.assertEquals(0, result.deleteFiles().length);
  ...
  IcebergStreamWriter createIcebergStreamWriter() {
  ...
  IcebergStreamWriter createStreamWriter(
      ...
      boolean newUpsert) {
    return FlinkSink.createStreamWriter(newTable, newFlinkRowType, newEqualityFieldIds, newUpsert, "1", 1, 1);
  }
}
