...
public class TestIcebergStreamWriter {
  ...
  public void testPreCommit() throws Exception {
    IcebergStreamWriter<RowData> icebergStreamWriter = createIcebergStreamWriter();
    icebergStreamWriter.write(SimpleDataUtil.createRowData(1, "hello"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(2, "hello"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(3, "hello"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(4, "hello"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(5, "hello"), new ContextImpl());

    Collection<IcebergFlinkCommittable> icebergFlinkCommittables = icebergStreamWriter.prepareCommit();
    Assert.assertEquals(icebergFlinkCommittables.size(), 1);
  }
  ...
  public void testWritingTable() throws Exception {
    ...
    icebergStreamWriter.write(SimpleDataUtil.createRowData(1, "hello"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(2, "world"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(3, "hello"), new ContextImpl());
    ...
    for (IcebergFlinkCommittable icebergFlinkCommittable : icebergFlinkCommittables) {
      result = icebergFlinkCommittable.committable();
    }
    ...
    icebergStreamWriter.write(SimpleDataUtil.createRowData(4, "foo"), new ContextImpl());
    icebergStreamWriter.write(SimpleDataUtil.createRowData(5, "bar"), new ContextImpl());

    icebergFlinkCommittables = icebergStreamWriter.prepareCommit();
    for (IcebergFlinkCommittable icebergFlinkCommittable : icebergFlinkCommittables) {
      result = icebergFlinkCommittable.committable();
    }
  ...
  public void testSnapshotAndRestore() throws Exception {
    ...
    Collection<IcebergStreamWriterState<RowData>> icebergStreamWriterStates = icebergStreamWriter.snapshotState(1L);
    icebergStreamWriter.prepareCommit();
    Assert.assertEquals(icebergStreamWriterStates.size(), 1);

    IcebergStreamWriter<RowData> icebergStreamWriter2 = createIcebergStreamWriter();
    IcebergStreamWriter<RowData> icebergStreamWriterRestore =
        icebergStreamWriter2.restoreWriter(icebergStreamWriterStates);

    List<IcebergStreamWriterState<RowData>> icebergStreamWriterStatesRestore =
        icebergStreamWriterRestore.snapshotState(2L);
    Assert.assertEquals(icebergStreamWriterStates.size(), icebergStreamWriterStatesRestore.size());
  }
  ...
  public void testPromotedFlinkDataType() throws Exception {
    ...
    for (RowData row : rows) {
      icebergStreamWriter.write(row, new ContextImpl());
    }
    Collection<IcebergFlinkCommittable> icebergFlinkCommittables = icebergStreamWriter.prepareCommit();
  ...
  IcebergStreamWriter<RowData> createIcebergStreamWriter() {
    return createIcebergStreamWriter(table, SimpleDataUtil.FLINK_SCHEMA);
  }

  IcebergStreamWriter<RowData> createIcebergStreamWriter(
      Table icebergTable, TableSchema flinkSchema) {
  ...
  IcebergStreamWriter<RowData> createStreamWriter(
      Table newTable,
      RowType newFlinkRowType,
      List<Integer> newEqualityFieldIds,
      boolean newUpsert) {
    return FlinkSink.createStreamWriter(newTable, newFlinkRowType, newEqualityFieldIds, newUpsert, 1, 1);
  }
}
