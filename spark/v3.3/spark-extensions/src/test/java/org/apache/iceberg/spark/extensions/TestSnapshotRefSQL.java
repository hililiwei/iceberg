...
public class TestSnapshotRefSQL extends SparkExtensionsTestBase {
  ...
  public void testUseBranch() throws NoSuchTableException {
    ...
    table.manageSnapshots().createBranch(branchName, table.currentSnapshot().snapshotId());
    ...
    assertEquals("Should have expected rows",
        ...
        sql("SELECT * FROM %s.%s", tableName, prefix + branchName));

    String mainBranch = "main";
    assertEquals("Should have expected rows",
        ImmutableList.of(row(1, "a"), row(2, "b"), row(1, "a"), row(2, "b")),
        sql("SELECT * FROM %s.%s", tableName, prefix + mainBranch));

    String branchNotExist = "b2";
    AssertHelpers.assertThrows("Cant not use a ref that doest not exist!",
        IllegalArgumentException.class, "Snapshot ref does not exist: b2",
        () -> sql("SELECT * FROM %s.%s", tableName, prefix + branchNotExist));
  ...
  public void testUseTag() throws NoSuchTableException {
    ...
    Table table = validationCatalog.loadTable(tableIdent);
    String tagName = "t1";
    table.manageSnapshots().createTag(tagName, table.currentSnapshot().snapshotId());
